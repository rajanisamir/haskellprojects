...
censor f writerT = WriterT $ (\(a, w) -> (a, f w)) <$> (runWriterT writerT)


instance Functor m => Functor (WriterT w m) where
    fmap :: Functor m => (a -> b) -> WriterT w m a -> WriterT w m b
    fmap f writerTArg = WriterT $
      (\(a, w) -> (f a, w)) <$> runWriterT writerTArg
...
instance (Monoid w, Applicative m) => Applicative (WriterT w m) where
    ...
    writerTFunc <*> writerTArg =
        WriterT $
          (\(f, w1) (a, w2) -> (f a, w1 <> w2))
          <$> runWriterT writerTFunc
          <*> runWriterT writerTArg
